...
void CTPPSPixelDataFormatter::interpretRawData(
  ...
  for (auto word = bw; word < ew; ++word) {
    ...
    if (row == 0 || row == 80) {
      std::cout << "[[[[[[[[[[[[[[[[[[[[[[     convroc, col, row " << convroc << ", " << col << ", " << row
                << "  <------ " << std::endl;
    } else {
      std::cout << "[[[[[[[[[[[[[[[[[[[[[[  "
                << "fedId=" << fedId << " nllink=" << nlink << " nroc=" << convroc << " adc=" << adc << "  col, row "
                << col << ", " << row << std::endl;
    }
    ...
    if (!isRun3 && (dcol < min_Dcol || dcol > max_Dcol || pxid < min_Pixid || pxid > max_Pixid)) {
      edm::LogError("CTPPSPixelDataFormatter")
          ...
          << "fedId=" << fedId << " nllink=" << nlink << " nroc=" << convroc << " adc=" << adc << " dcol=" << dcol
          << " pxid=" << pxid << " detId=" << iD;
    ...
    if (isRun3 && (col < min_COL || col > max_COL || row < min_ROW || row > max_ROW)) {
      edm::LogError("CTPPSPixelDataFormatter") << " unphysical col and/or row "
                                               << "fedId=" << fedId << " nllink=" << nlink << " nroc=" << convroc
